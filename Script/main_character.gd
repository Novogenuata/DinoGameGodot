...
@onready var Actfinder_collision = %ActionableFinder/CollisionShape2D
...
@onready var weapon3 = %gun3  # Water gun

var weapon_index: int = 0  # 0 = weapon1, 1 = weapon2, 2 = weapon3
...
func _ready():
	...
	if not Globalmanager.has_water_gun:
...
func get_input():
	...
	if input_dir.length() > 0:
		...
		if input_dir.x > 0:
			sprite.flip_h = true
			finder.position.x = -finder_original_x
			Actfinder.position.x = -actfinder_original_x
		elif input_dir.x < 0:
			sprite.flip_h = false
			finder.position.x = finder_original_x
			Actfinder.position.x = actfinder_original_x
	...
	if Input.is_action_just_pressed("switch"):
		var total_weapons = 2
		if Globalmanager.has_water_gun:
			total_weapons = 3
		weapon_index = (weapon_index + 1) % total_weapons
		_update_weapon_visibility()
...
func _update_weapon_visibility() -> void:
	...
	for i in range(weapons.size()):
		...
		var is_active = (i == weapon_index and weapon != null and (i < 2 or Globalmanager.has_water_gun))
		if weapon:
